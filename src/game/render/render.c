...
void	render_map(t_cub3d *cub3d)
{
	...
	if (!cub3d->map.img)
		handle_close(cub3d, "Failed to load map_img");
	draw_minimap(*cub3d);
	...
	if (!cub3d->ray_caster.img)
		handle_close(cub3d, "Failed to create ray_caster image");
...
void render_player(t_cub3d *cub3d)
{
	...
	if (!cub3d->player.img)
		handle_close(cub3d, "Player image creation failed");
	draw_player(*cub3d);
	...
	if (mlx_image_to_window(cub3d->mlx, cub3d->player.img, img_x, img_y) < 0)
		handle_close(cub3d, "Failed to draw player");
}
