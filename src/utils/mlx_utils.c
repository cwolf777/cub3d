...
{
	cub3d->map.img = mlx_new_image(cub3d->mlx, cub3d->minimap_img_width , cub3d->minimap_img_height);
	if (!cub3d->map.img)
		handle_error(cub3d, "Failed to load map_img");
	cub3d->ray_caster.img = mlx_new_image(cub3d->mlx, cub3d->minimap_img_width , cub3d->minimap_img_height);
...
{
	cub3d->view_img = mlx_new_image(cub3d->mlx, cub3d->window_width, cub3d->window_height);
	if (!cub3d->view_img)
...
void setup_mlx(t_cub3d *cub3d)
{
	...
	mlx_set_window_limit(cub3d->mlx, MIN_WINDOW_WIDTH, MIN_WINDOW_HEIGHT, MAX_WINDOW_WIDTH, MAX_WINDOW_HEIGHT);
}
